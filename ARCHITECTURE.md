...
The video quality control messages (`protocol/schemas/video-control-request.schema.json`, `video-control-state.schema.json`) are deliberately shaped as a `control`-discriminated envelope, with only `"quality"` implemented today. Future ground-initiated video controls — camera source selection, zoom, gain/contrast — are expected to be added as new `control` cases in `air-client-sdk`'s `WebRtcSessionManager` and new GUI widgets, without renaming the protocol or changing `ground-client-sdk`'s relay code, which stays agnostic to the payload shape.
