...
The video control messages (`protocol/schemas/video-control-request.schema.json`, `video-control-state.schema.json`) are deliberately shaped as a `control`-discriminated envelope, with `"quality"` and `"camera-source"` implemented today (plus `"camera-source-list"`, air's unsolicited device-list push, on the state side only). Further ground-initiated video controls — zoom, gain/contrast — are expected to be added as new `control` cases in `air-client-sdk`'s `WebRtcSessionManager` and new GUI widgets, without renaming the protocol or changing `ground-client-sdk`'s relay code, which stays agnostic to the payload shape: adding camera-source required zero changes to `ground-client-sdk`, confirming the design.
