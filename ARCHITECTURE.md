...
- Ground-initiated video flip (horizontal/vertical mirroring), for when the phone can't be mounted in the correct orientation: the ground GUI's two checkboxes send a `control: "flip"` request carrying both flags; air redraws the camera feed onto a canvas with a mirrored 2D transform (same width/height as the source, so no resolution desync) and swaps the canvas's `captureStream()` track into the sender via `replaceVideoTrack`, since raw camera tracks and `RTCRtpSender` have no flip of their own
...
The video control messages (`protocol/schemas/video-control-request.schema.json`, `video-control-state.schema.json`) are deliberately shaped as a `control`-discriminated envelope, with `"quality"`, `"camera-source"`, and `"flip"` implemented today (plus `"camera-source-list"`, air's unsolicited device-list push, on the state side only). Further ground-initiated video controls — zoom, gain/contrast — are expected to be added as new `control` cases in `air-client-sdk`'s `WebRtcSessionManager` and new GUI widgets, without renaming the protocol or changing `ground-client-sdk`'s relay code, which stays agnostic to the payload shape: adding camera-source and flip each required zero changes to `ground-client-sdk`, confirming the design.

Camera capture (`getUserMedia`, device enumeration) is owned by `air-webapp`'s app layer, not `air-client-sdk` — the SDK stays transport-only and exposes `setCameraSourceHandler()` for the app to register its own switch logic (`switchCameraTo()` in `app.ts`), the same separation the local device-picker dropdown already relied on. Flip follows the same split: `setFlipHandler()` lets the app own the actual frame transform (`video-flip.ts`'s canvas re-render pipeline) while the SDK only dispatches the request and reports the handler's ok/error outcome.
